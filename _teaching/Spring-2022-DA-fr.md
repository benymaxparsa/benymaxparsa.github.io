---
title: "Co-auxiliaire d'enseignement principal du cours de conception et analyse d'algorithmes"
collection: teaching
type: "Cours de premier cycle"
permalink: /fr/enseignement/Spring-2022-DA
venue: "Université Vali-e-Asr de Rafsanjan, Département de génie informatique"
date: 2022-02-01
location: "Rafsanjan, Kerman, Iran"
lang: fr-CA
---

- Cours : Conception et analyse d'algorithmes
- Professeure du cours : Dr Fahimeh Dabaghi-Zarandi
- Session : Printemps 2022
- Taille de l'équipe : 4 auxiliaires
- Mon rôle clé consistait à :
  - Enseigner différents algorithmes aux étudiants de premier cycle en génie informatique
  - Corriger les devoirs des étudiants
  - Tenir des cours de récitation
  - Gérer et organiser les autres auxiliaires avec l'aide de l'autre auxiliaire principal
  - Concevoir des devoirs et projets pour les étudiants
  - Consulter le professeur et planifier ensemble le programme du cours avec l'aide de l'autre auxiliaire principal
  - Aider et guider les étudiants pour qu'ils réussissent dans le cours qu'ils ont suivi

Compétences apprises et utilisées : Conception d'algorithmes · Enseignement · python · Théorie des graphes · Gestion d'équipe · Analyse d'algorithmes · Structures de données · Programmation orientée objet (POO) · Tests unitaires · C++ · LaTeX · Git · Résolution de problèmes · Leadership d'équipe · Travail d'équipe
